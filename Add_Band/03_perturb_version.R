...
plot_error_turnout = function(d, m) {
    ggplot(d, aes(dem, fitted(m), size = voters, color = turnout)) +
        ...
        scale_y_continuous(limits = c(-30, 30), expand = expansion(mult = 0)) +
...
plot_error_hh = function(d, m) {
    d %>%
        mutate(largest_race = recode_factor(max_race,
               ...
               #error = fitted(m)) %>%
        sample_frac(1) %>%
    ggplot(aes(hh, perturb_error, size=voters, color = largest_race)) +
        geom_hline(yintercept=0, lty="dashed") +
...
state_plots = map2(state_errors, state_models, plot_error_turnout) %>%
    map2(c("Pennsylvania", "North Carolina", "South Carolina", "Louisiana",
         ...
         ~ .x + ggtitle(.y))
state_plots$PA = state_plots$PA + labs(y = "Fitted Perturbed DAS-12.2 population error")
state_plots$AL = state_plots$AL + labs(y = "Fitted Perturbed DAS-12.2 population error")
