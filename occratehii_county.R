...
county_hhidp <- allcountydata_122 %>%
...
county_hhi<- left_join(county_hhisf,county_hhidp, by='countycode') %>%
  mutate (hhi_diff = hhi_sf - hhi_dp,.after=countycode)

#--Join Data Frame--------------------------------------------------------
county_vrhhi <- left_join(county_hhi, county_vr, by='countycode')%>%
  mutate (x1test=hhi_sf*100,
          ytest =occupancyrate_diff*100 )

#--Fit model and Plot----------------------------------------------
mod1<-gam(ytest ~  s(x1test), data= county_vrhhi)
fit1<-fitted(mod1)
range(fit1)
plot(fit1~county_vrhhi$x1test,
     ylab = "Fitted Occupancy Rate Difference",
     xlab = "Herfindahl-Hirschman Index",
     main= "Model-smoothed Occupancy Rate Error in county level by the Herfindahl-Hirschman index",
     cex.main=0.8) 
